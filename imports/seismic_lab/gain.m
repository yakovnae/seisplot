...
%     
 ...
 [nt,nx] = size(d);
 dout = zeros(size(d)); % 20191104 YAKOV NAE: Memory alocation
 ...
 if strcmp(option1,'time');   % Geometrical spreading-like gain
  ...
  t = (0:1:nt-1)*dt; 
  tgain = (t.^a).*exp(b.*t);
  dout = zeros(nt,nx); % 20191104 YAKOV NAE: Memory alocation
 ...
 if option2==1;                % Normalize by amplitude 
   ...
   for k = 1:nx
    aux =  dout(:,k);
    amax = max(abs(aux));
    dout(:,k) = dout(:,k)/(amax+eps); % 20191104 YAKOV NAE: Zero division
   end
