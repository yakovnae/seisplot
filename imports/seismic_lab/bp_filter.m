...
function  [o] =  bp_filter(d,dt,filter)
...
%
    if isstruct(d)
        ...
        for ii=1:length(fnames)
            o.(fnames{ii}) = local_bp_filter(d.(fnames{ii}),dt,filter);
        end
    else
        o =  local_bp_filter(d,dt,filter);
    end
    
end
function  [o] =  local_bp_filter(d,dt,filter)
